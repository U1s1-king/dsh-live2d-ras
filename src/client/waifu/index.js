...
function enableDrag(widgetEl) {
    ...
    const endDrag = event => {
        ...
        if (drag.moved) {
            const rect = widgetEl.getBoundingClientRect();
            try {
                localStorage.setItem("ras-waifu-pos", JSON.stringify({ left: rect.left, top: rect.top }));
            } catch (error) { /* 忽略 */ }
        }
    };
...
async function initWidget(config) {
    ...
    if (localStorage.getItem("ras-waifu-display") && Date.now() - localStorage.getItem("ras-waifu-display") <= 86400000) {
