...
/**
* RAISE A SUILEN Live2D 桌宠 —— host 半区路由。
* 同源静态路由 /ras-assets/* 暴露插件 assets 目录（vendor 运行时脚本、
...
const PET_ASSET_PREFIX = "/ras-assets";
const MIME = {
...
/** 静态资源前缀路由：/ras-assets/<assets 内相对路径>。 */
function makeAssetRoute(packageRoot) {
	return {
		...
		handler: (req, res) => {
			...
			const abs = assetPath(packageRoot, urlPath.startsWith("/ras-assets") ? urlPath.slice(11) : urlPath);
			if (abs === null) {
				res.writeHead(400);
				res.end();
				return;
...
const name = "dsh-live2d-ras";
/** 注册静态资源路由；插件卸载时自动清理。 */
function apply(ctx) {
	const packageRoot = packageRootOf(import.meta.url);
	ctx.inject(["webServer"], (httpCtx) => {
		httpCtx.effect(() => httpCtx.webServer.register(makeAssetRoute(packageRoot)), "live2d-ras: routes");
	});
}
//#endregion
