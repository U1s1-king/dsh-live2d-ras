window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let modelId = readStoredId("ras-modelId");
		let modelTexturesId = readStoredId("ras-modelTexturesId");
		let config = {};
		let messageArray = [];
		function getModelId() {
		...
		function setModelId(newModelId) {
			modelId = newModelId;
			localStorage.setItem("ras-modelId", newModelId.toString());
		}
		...
		function setModelTexturesId(newModelTexturesId) {
			...
			localStorage.setItem("ras-modelTexturesId", newModelTexturesId.toString());
		}
		function resetModelState() {
			modelId = 0;
			modelTexturesId = 0;
			localStorage.setItem("ras-modelId", "0");
			localStorage.setItem("ras-modelTexturesId", "0");
		}
		function getConfig() {
			return config;
		}
		...
		function showMessage(model, text, timeout, priority) {
			if (!text) return;
			const storedPriority = parseInt(sessionStorage.getItem("ras-waifu-text"), 10);
			...
			sessionStorage.setItem("ras-waifu-text", priority);
			const tips = document.getElementById("waifu-tips-ras");
			if (tips) {
    ...
    <div id="texture-selection-panel-ras" class="waifu-panel waifu-panel-ras" style="display: none;"></div>`);
			...
			for (const tool of getConfig().tools) {
				...
				toolBar.insertAdjacentHTML("beforeend", `<span id="waifu-tool-ras-${tool}" title="${TOOL_TITLES[tool] || tool}">${decodeURIComponent(icon).replace("data:image/svg+xml,", "")}</span>`);
			...
			hooks.on(document, "click", (event) => {
				if (event.target.closest("#model-selection-panel-ras") || event.target.closest("#texture-selection-panel-ras") || event.target.closest("#waifu-tool-ras") || event.target.closest("#waifu-toggle-ras")) return;
				closePanels();
			});
		...
		function enableDrag(widgetEl) {
			...
			widgetEl.addEventListener("pointerdown", (event) => {
				if (event.target.closest("#waifu-tool-ras") || event.target.closest(".waifu-panel-ras") || event.target.closest("#waifu-toggle-ras")) return;
				drag.active = true;
				drag.moved = false;
...
#waifu-ras, #waifu-toggle-ras { z-index: 2147483646 !important; }
.waifu-panel { z-index: 2147483647 !important; }
#waifu-ras { left: auto; right: 900px; top: auto; bottom: 20px; }
`;
			...
			ctx.effect(() => {
				...
				(async () => {
					...
					try {
						...
						console.error("[live2d-ras] 桌宠启动失败", error);
					}
				})();
				return stop;
			}, "live2d-ras: widget");
